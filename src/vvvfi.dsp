import("stdfaust.lib");

sgn = _ <: >(0) - <(0);
rnd = (no.noise-0.5)*hslider("Random Mod", 0, 0, 100, 0.01)*20;

freq = hslider("freq[style:radio{}]", 0, 0, 0, 0);
gate = hslider("gate[style:radio{}]", 0, 0, 0, 0);

sync = checkbox("Sync");
fullphs = checkbox("Full Phases");
mf = hslider("Mod Freq", 0, 0, 100, 0.01) * ba.if(sync, freq/100, 1);
mlv = hslider("Mod Level", 0, 0, 100, 0.01)/100;
cff = hslider("Car Freq Factor", 0, 0, 100, 0.01)/100;
cf = ba.if(sync, (floor(cff*14)*2-1)*mf, freq*2^floor(cff*3));

phasor(freq) = (+(freq/ma.SR) ~ ma.decimal);

car_osc = asin(sin(phasor(cf+rnd)*(ma.PI*2)))/(ma.PI/2);
mod_osc(freq, ph) = sin((ph+phasor(freq))*(ma.PI*2)) * mlv;

vlt1 = sgn(mod_osc(mf, 0)-car_osc) * fullphs;
vlt2 = sgn(mod_osc(mf, 1/3)-car_osc) - vlt1;
vlt3 = sgn(mod_osc(mf, 2/3)-car_osc) - vlt1;

process = (vlt2-vlt3)*gate;