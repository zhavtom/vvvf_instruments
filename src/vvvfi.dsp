...
mlv = g_osc(hslider("[3]Mod Level", 0, 0, 100, 0.1))/100;
...
mf = mff * ba.if(sync, freq, 128);
cf = ba.if(sync, pls*mf, freq*(2^(bend/12))*128);

hs = os.phasor(1, mf/512) > 0.5 <: _ == _@(1) : ba.if(sync, _, 1);
...
car_osc = base_osc((cf+ssp_s)*ba.if(hfsi > 0, 0.5, 1), rnd_s) * (1+plv) / (pinv*4-1);
mod_osc(ph) = base_osc(mf, ph) * mlv * hfsi_s * (1+plv+sync*plv*2);
