...
cff = g_osc(hslider("[0]Car Freq / Pulse Mode", 33, 0, 100, 0.01)/100);
...
fphs = g_opt(hslider("[3]Phases", 2, 2, 3, 1))-2;

gain = g_ctl(vslider("[2]Gain", 50, 0, 100, 0.1))/100;
...
car_osc = base_osc((cf+rnd_s+ssp_s)/(ba.if(hfsi > 0, 2, 1)), 0);
...
process = ((v-w)*fphs-(u-v))/(1+fphs)*gain*gate;